...
var X,Y;
class BWOH2 extends React.Component {
    constructor(props) {
        super(props);
        X=[];
        Y=[];
        ...
        this.BWOH2=this.BWOH2.bind(this); //ประกาศฟังก์ชัน BWOH2
    }
    handleChange(event) {
    ...
    handleSubmit(event) {
        ...
        if((str.length!==0)&&(c!==0)&&this.state.submitted&&this.state.x!==""&&this.state.h!==""){ //เช็คความยาวสมการต้องไม่เท่ากับ 0 และ c ที่เช็ค x ต้องไม่เท่ากับ 0 และ xl xr ต้องไม่ใช่ค่าว่าง
          axios
          .post("http://localhost:8000/BWOH2/BWOH2", {
          ...
          this.BWOH2(); //ส่งไปที่ฟังก์ชัน BWOH2
    ...
    BWOH2(event){
      ...
      if(n === 1) {
        data.fxi = (3 * (func(x)) - (4 * (func(xi))) - (func(xi2))) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=1')
      }
      if(n === 2) {
        data.fxi = (2 * (func(x)) - (5 * (func(xi))) + (4 * (func(xi2))) - (func(xi3))) / (math.pow(h, n).toFixed(6))
        console.log('n=2')
      }
      if(n === 3) {
        data.fxi = (5 * (func(x)) - (18 * (func(xi))) + (24 * (func(xi2))) - (14 * (func(xi3))) - (func(xi4))) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=3')
      }
      if(n === 4) {
        data.fxi = (3 * (func(x)) - (14 * (func(xi))) + (26 * (func(xi2))) - (24 * (func(xi3))) + (11 * (func(xi4))) - (2 * (func(xi5)))) / (math.pow(h, n).toFixed(6))
        console.log('n=4')
      }
