...
var X,Y;
class OH2 extends React.Component {
    constructor(props) {
        super(props);
        X=[];
        Y=[];
        ...
        this.OH2=this.OH2.bind(this); //ประกาศฟังก์ชัน OH2
    }
    handleChange(event) {
    ...
    handleSubmit(event) {
        ...
        if((str.length!==0)&&(c!==0)&&this.state.submitted&&this.state.x!==""&&this.state.h!==""){ //เช็คความยาวสมการต้องไม่เท่ากับ 0 และ c ที่เช็ค x ต้องไม่เท่ากับ 0 และ xl xr ต้องไม่ใช่ค่าว่าง
          axios
          .post("http://localhost:8000/OH2/OH2", {
    ...
    OH2(event){
      ...
      if(n === 1) {
        data.fxi = (func(xi) - func(xi3)) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=1')
      }
      if(n === 2) {
        data.fxi = (func(xi) - (2 * (func(x))) + func(xi3)) / (math.pow(h, n).toFixed(6))
        console.log('n=2')
      }
      if(n === 3) {
        data.fxi = (func(xi2) - (2 * (func(xi))) + (2 * (func(xi3))) - func(xi4)) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=3')
      }
      if(n === 4) {
        data.fxi = (func(xi2) - (4 * (func(xi))) + (6 * (func(xi))) - (4 * (func(xi3))) - func(xi4)) / (math.pow(h, n).toFixed(6))
        console.log('n=4')
      }
