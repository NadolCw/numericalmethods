...
var X,Y;
class OH4 extends React.Component {
    constructor(props) {
        super(props);
        X=[];
        Y=[];
        ...
        this.OH4=this.OH4.bind(this); //ประกาศฟังก์ชัน OH4
    }
    handleChange(event) {
    ...
    handleSubmit(event) {
        ...
        if((str.length!==0)&&(c!==0)&&this.state.submitted&&this.state.x!==""&&this.state.h!==""){ //เช็คความยาวสมการต้องไม่เท่ากับ 0 และ c ที่เช็ค x ต้องไม่เท่ากับ 0 และ xl xr ต้องไม่ใช่ค่าว่าง
          axios
          .post("http://localhost:8000/OH4/OH4", {
    ...
    OH4(event){
      ...
      if(n === 1) {
        data.fxi = (-func(xi2) + (8 * (func(xi))) - (8 * (func(xi4))) + (func(xi5))) / (12*(math.pow(h, n).toFixed(6)))
        console.log('n=1')
      }
      if(n === 2) {
        data.fxi = (-func(xi2) + (16 * (func(xi))) - (30 * (func(x))) + (16 * (func(xi4))) - (func(xi5))) / (12*(math.pow(h, n).toFixed(6)))
        console.log('n=2')
      }
      if(n === 3) {
        data.fxi = (-func(xi3) + (8 * (func(xi2))) - (13 * (func(xi))) + (13 * (func(xi4))) - (8 * (func(xi5))) + (func(xi6))) / (8*(math.pow(h, n).toFixed(6)))
        console.log('n=3')
      }
      if(n === 4) {
        data.fxi = (-func(xi3) + (12 * (func(xi2))) - (39 * (func(xi))) + (56 * (func(x))) - (39 * (func(xi4))) + (12 * (func(xi5))) - (func(xi6))) / (6*(math.pow(h, n).toFixed(6)))
        console.log('n=4')
      }
