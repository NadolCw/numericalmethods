...
var X,Y;
class FWOH2 extends React.Component {
    constructor(props) {
        super(props);
        X=[];
        Y=[];
        ...
        this.FWOH2=this.FWOH2.bind(this); //ประกาศฟังก์ชัน FWOH2
    }
    handleChange(event) {
    ...
    handleSubmit(event) {
        ...
        if((str.length!==0)&&(c!==0)&&this.state.submitted&&this.state.x!==""&&this.state.h!==""){ //เช็คความยาวสมการต้องไม่เท่ากับ 0 และ c ที่เช็ค x ต้องไม่เท่ากับ 0 และ xl xr ต้องไม่ใช่ค่าว่าง
          axios
          .post("http://localhost:8000/FWOH2/FWOH2", {
          ...
          this.FWOH2(); //ส่งไปที่ฟังก์ชัน FWOH2
    ...
    FWOH2(event){
      ...
      if(n === 1) {
        data.fxi = (-func(xi2) + (4 * (func(xi))) - (3 * func(x))) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=1')
      }
      if(n === 2) {
        data.fxi = (-func(xi3) + (4 * (func(xi2))) - (5 * (func(xi))) + (2 * (func(x)))) / (math.pow(h, n).toFixed(6))
        console.log('n=2')
      }
      if(n === 3) {
        data.fxi = (-3 * (func(xi4)) + (14 * (func(xi3))) - (24 * (func(xi2))) + (18 * (func(xi))) - (5 * (func(x)))) / (2*(math.pow(h, n).toFixed(6)))
        console.log('n=3')
      }
      if(n === 4) {
        data.fxi = (-2 * (func(xi5)) + (11 * (func(xi4))) - (24 * (func(xi3))) + (26 * (func(xi2))) - (14 * (func(xi))) + (3 * (func(x)))) / (math.pow(h, n).toFixed(6))
        console.log('n=4')
      }
